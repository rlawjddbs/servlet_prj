...
public class WebMemberJoin extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 사용자가 입력한 값(HTML Form Control) 받기(getParameter, getParameterValues)
		...
		// 파라메터 값을 VO에 저장
		WebMemberVO wm_vo = new WebMemberVO(id, ShaUtil.shaEncoding(pass1), name, 
				ShaUtil.shaEncoding(ssn1+"-"+ssn2), gender, b_year+"-"+b_month+"-"+b_day,age, marriageFlag, 
				m_year+"-"+m_month+"-"+m_day, zipcode, addr1, addr2, 
				greetings, request.getRemoteAddr(), interestlang);
		...
		try {
			// 비 연결성을 해결하기 위해
			// 추가전에 아이디를 사용중인지 다시 재조회를 하여 처리한다.
		...
		out.print(b_year+"년 "+b_month+"월 "+b_day+"일</strong></li>\r\n");
		...
		out.write("\t</div>\r\n");
			...
			out.println("서비스가 원활하지 못한 점 죄송합니다..<br />");
			out.println("정상적인 서비스를 위해서 저희 임직원들은 최선을 다하고 있습니다.<br />");
			out.println("신속하게 복구하겠습니다. 사용 중 불편한점은 010-디잘잘에게 연락해주세요.<br />");
		} // end catch
	...
	private void addWebMember(WebMemberVO wm_vo) throws SQLException, RuntimeException{
		...
		// 추가전에 동일 아이디가 존재하는지 다시 검색하여 처리 (비연결성에 대한 처리)
